...
async function findLibraryItemByItemToFileInoMatch(libraryId, fullPath, isSingleMedia) {
  ...
  const existingLibraryItem = await Database.libraryItemModel.findOneExpanded(
    [
      ...
      sequelize.where(sequelize.literal(`(SELECT count(*) FROM ${Database.jsonArrayElements('libraryFiles')} WHERE ${Database.jsonValid('libraryFiles')} AND json_each.value->>'$.ino' = :inode)`), {
        [sequelize.Op.gt]: 0
      })
    ],
    {
