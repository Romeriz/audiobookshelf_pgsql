...
module.exports = {
  ...
  async search(user, library, query, limit, offset) {
    ...
    // Search genres
    const genreMatches = []
    const [genreResults] = await Database.sequelize.query(`SELECT ${Database.jsonEachText('value')} AS value, count(*) AS numItems FROM ${Database.getTableName('books')} b, ${Database.getTableName('libraryItems')} li, ${Database.jsonArrayElements('b.genres')} WHERE ${Database.jsonValid(Database.getColumnRef('b', 'genres'))} AND ${matchJsonValue} AND ${Database.getColumnRef('b', 'id')} = ${Database.getColumnRef('li', 'mediaId')} AND ${Database.getColumnRef('li', 'libraryId')} = :libraryId GROUP BY ${Database.jsonEachText('value')} ORDER BY numItems DESC LIMIT :limit OFFSET :offset;`, {
      replacements: {
        libraryId: library.id,
        limit,
        offset
  ...
  async getGenresWithCount(libraryId) {
    const genres = []
    const [genreResults] = await Database.sequelize.query(`SELECT ${Database.jsonEachText('value')} AS value, count(*) AS numItems FROM ${Database.getTableName('books')} b, ${Database.getTableName('libraryItems')} li, ${Database.jsonArrayElements('b.genres')} WHERE ${Database.jsonValid(Database.getColumnRef('b', 'genres'))} AND ${Database.getColumnRef('b', 'id')} = ${Database.getColumnRef('li', 'mediaId')} AND ${Database.getColumnRef('li', 'libraryId')} = :libraryId GROUP BY ${Database.jsonEachText('value')} ORDER BY numItems DESC;`, {
      replacements: {
        libraryId
      },
      raw: true
