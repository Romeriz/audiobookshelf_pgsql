...
module.exports = {
  ...
  async getFilteredSeries(library, user, filterBy, sortBy, sortDesc, include, limit, offset) {
    ...
    if (library.settings.hideSingleBookSeries) {
      seriesWhere.push(
        Sequelize.where(Sequelize.literal(`(SELECT count(*) FROM ${Database.getTableName('books')} b, ${Database.getTableName('bookSeries')} bs WHERE ${Database.getColumnRef('bs', 'seriesId')} = ${Database.getColumnRef('series', 'id')} AND ${Database.getColumnRef('bs', 'bookId')} = ${Database.getColumnRef('b', 'id')})`), {
          [Sequelize.Op.gt]: 1
        })
      )
    }
    ...
    if (['genres', 'tags', 'narrators'].includes(filterGroup)) {
      ...
      attrQuery = `SELECT count(*) FROM ${Database.getTableName('books')} b, ${Database.getTableName('bookSeries')} bs WHERE ${Database.getColumnRef('bs', 'seriesId')} = ${Database.getColumnRef('series', 'id')} AND ${Database.getColumnRef('bs', 'bookId')} = ${Database.getColumnRef('b', 'id')} AND ${Database.getColumnRef('b', 'publisher')} = :filterValue`
      userPermissionBookWhere.replacements.filterValue = filterValue
    } else if (filterGroup === 'languages') {
      attrQuery = `SELECT count(*) FROM ${Database.getTableName('books')} b, ${Database.getTableName('bookSeries')} bs WHERE ${Database.getColumnRef('bs', 'seriesId')} = ${Database.getColumnRef('series', 'id')} AND ${Database.getColumnRef('bs', 'bookId')} = ${Database.getColumnRef('b', 'id')} AND ${Database.getColumnRef('b', 'language')} = :filterValue`
      userPermissionBookWhere.replacements.filterValue = filterValue
      ...
      if (filterValue === 'not-finished') {
        ...
        const progQuery = `SELECT count(*) FROM ${Database.getTableName('books')} b, ${Database.getTableName('bookSeries')} bs LEFT OUTER JOIN ${Database.getTableName('mediaProgresses')} mp ON ${Database.getColumnRef('mp', 'mediaItemId')} = ${Database.getColumnRef('b', 'id')} AND ${Database.getColumnRef('mp', 'userId')} = :userId WHERE ${Database.getColumnRef('bs', 'seriesId')} = ${Database.getColumnRef('series', 'id')} AND ${Database.getColumnRef('bs', 'bookId')} = ${Database.getColumnRef('b', 'id')} AND (${Database.getColumnRef('mp', 'isFinished')} IS NULL OR ${Database.getColumnRef('mp', 'isFinished')} = ${Database.bool(false)})`
    ...
    if (userPermissionBookWhere.bookWhere.length) {
      if (!attrQuery) attrQuery = `SELECT count(*) FROM ${Database.getTableName('books')} b, ${Database.getTableName('bookSeries')} bs WHERE ${Database.getColumnRef('bs', 'seriesId')} = ${Database.getColumnRef('series', 'id')} AND ${Database.getColumnRef('bs', 'bookId')} = ${Database.getColumnRef('b', 'id')}`
    ...
    if (sortBy === 'numBooks') {
      ...
      seriesAttributes.include.push([Sequelize.literal(`(SELECT SUM(${Database.getColumnRef('b', 'duration')}) FROM ${Database.getTableName('books')} b, ${Database.getTableName('bookSeries')} bs WHERE ${Database.getColumnRef('bs', 'seriesId')} = ${Database.getColumnRef('series', 'id')} AND ${Database.getColumnRef('b', 'id')} = ${Database.getColumnRef('bs', 'bookId')})`), 'totalDuration'])
      order.push(['totalDuration', dir])
    } else if (sortBy === 'lastBookAdded') {
      seriesAttributes.include.push([Sequelize.literal(`(SELECT MAX(${Database.getColumnRef('b', 'createdAt')}) FROM ${Database.getTableName('books')} b, ${Database.getTableName('bookSeries')} bs WHERE ${Database.getColumnRef('bs', 'seriesId')} = ${Database.getColumnRef('series', 'id')} AND ${Database.getColumnRef('b', 'id')} = ${Database.getColumnRef('bs', 'bookId')})`), 'mostRecentBookAdded'])
      order.push(['mostRecentBookAdded', dir])
    } else if (sortBy === 'lastBookUpdated') {
      seriesAttributes.include.push([Sequelize.literal(`(SELECT MAX(${Database.getColumnRef('b', 'updatedAt')}) FROM ${Database.getTableName('books')} b, ${Database.getTableName('bookSeries')} bs WHERE ${Database.getColumnRef('bs', 'seriesId')} = ${Database.getColumnRef('series', 'id')} AND ${Database.getColumnRef('b', 'id')} = ${Database.getColumnRef('bs', 'bookId')})`), 'mostRecentBookUpdated'])
