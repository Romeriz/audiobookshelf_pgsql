...
module.exports = {
  ...
  async getSeriesMostRecentlyAdded(library, user, include, limit) {
    ...
    if (userPermissionBookWhere.bookWhere.length) {
      let attrQuery = `SELECT count(*) FROM ${Database.getTableName('books')} b, ${Database.getTableName('bookSeries')} bs WHERE ${Database.getColumnRef('bs', 'seriesId')} = ${Database.getColumnRef('series', 'id')} AND ${Database.getColumnRef('bs', 'bookId')} = ${Database.getColumnRef('b', 'id')}`
      ...
      if (!user.permissions?.accessAllTags && user.permissions?.itemTagsSelected?.length) {
        if (user.permissions.selectedTagsNotAccessible) {
          attrQuery += ` AND (SELECT count(*) FROM ${Database.jsonArrayElements(Database.getColumnRef('b', 'tags'))} WHERE ${Database.jsonValid(Database.getColumnRef('b', 'tags'))} AND json_each.value IN (:userTagsSelected)) = 0`
        } else {
          attrQuery += ` AND (SELECT count(*) FROM ${Database.jsonArrayElements(Database.getColumnRef('b', 'tags'))} WHERE ${Database.jsonValid(Database.getColumnRef('b', 'tags'))} AND json_each.value IN (:userTagsSelected)) > 0`
        }
      }
