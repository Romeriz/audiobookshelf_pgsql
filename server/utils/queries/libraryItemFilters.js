...
module.exports = {
  ...
  async getAllLibraryItemsWithTags(tags) {
    ...
    const booksWithTag = await Database.bookModel.findAll({
      where: Sequelize.where(Sequelize.literal(`(SELECT count(*) FROM ${Database.jsonArrayElements('tags')} WHERE ${Database.jsonValid('tags')} AND json_each.value IN (:tags))`), {
        [Sequelize.Op.gte]: 1
      }),
      replacements: {
        tags
  ...
  async getAllLibraryItemsWithGenres(genres) {
    ...
    const booksWithGenre = await Database.bookModel.findAll({
      where: Sequelize.where(Sequelize.literal(`(SELECT count(*) FROM ${Database.jsonArrayElements('genres')} WHERE ${Database.jsonValid('genres')} AND json_each.value IN (:genres))`), {
        [Sequelize.Op.gte]: 1
      }),
      replacements: {
        genres
  ...
  async getAllLibraryItemsWithNarrators(narrators) {
    const libraryItems = []
    const booksWithGenre = await Database.bookModel.findAll({
      where: Sequelize.where(Sequelize.literal(`(SELECT count(*) FROM ${Database.jsonArrayElements('narrators')} WHERE ${Database.jsonValid('narrators')} AND json_each.value IN (:narrators))`), {
        [Sequelize.Op.gte]: 1
      }),
      replacements: {
        narrators
